...
void RandBot::send_nme_or_obs( void )
{
	...
	if ( m_map_and_units->game_over == false )
	{
		...
		else
		{
			...
			else if ( m_map_and_units->our_units.size() < m_map_and_units->our_centres.size() )
			{
				...
				{
					build_location.province_index = get_random_set_member<MapAndUnits::PROVINCE_SET>( m_map_and_units->open_home_centres );
					build_coast_info = &( m_map_and_units->game_map[ build_location.province_index ].coast_info );
					build_location.coast_token = get_random_set_member<MapAndUnits::PROVINCE_COASTS>( *build_coast_info ).first;
