...
void Socket::SendData() {
	...
	for (;;) { // while data available to send and space avalable
		...
		int sent = send(MySocket, OutgoingMessage+OutgoingNext, OutgoingLength-OutgoingNext, 0); // # bytes sent, or SOCKET_ERROR
...
void Socket::ReceiveData() {
	...
	for (;;) { // while incoming data available
		int count = min(IncomingLength-IncomingNext, received-bufferNext); // max # bytes that may be read into current message
		if (!count) return; // no more data available

		ASSERT(count > 0);

		memcpy(IncomingMessage+IncomingNext, buffer+bufferNext, count); // copy `count` bytes from `buffer` to IncomingMessage
		IncomingNext += count;
		bufferNext += count;
...
char* Socket::PullIncomingMessage(void) {
	...
	if (!IncomingMessageQueue.empty()) PostMessage(main_wnd, WM_SOCKET_MESSAGE, 0, 0); // trigger recall if more messages remain
