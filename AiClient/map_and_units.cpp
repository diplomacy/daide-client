...
int MapAndUnits::process_now_unit( TokenMessage unit_message )
{
	...
	if ( error_location == ADJUDICATOR_NO_ERROR )
	{
		...
		else if ( game_map[ province.get_subtoken() ].coast_info.find( coast ) == game_map[ province.get_subtoken() ].coast_info.end() )
		{
			error_location = 2;
		}
		else
		{
			...
			if ( unit_message.get_submessage_count() == 5 )
			{
				...
				else
				{
					...
					{
						unit.retreat_options.insert( get_coast_id( retreat_option_list.get_submessage( retreat_location_counter ), unit_type ) );
					}
				}
	...
	else
	{
		...
		if ( season == TOKEN_SEASON_WIN )
		{
			...
			if ( adjustment_orders_iterator == last_adjustment_results.end() )
			{
				...
				adjustment_orders_iterator = last_adjustment_results.insert( WINTER_ORDERS::value_type( power, new_adjustment_orders ) ).first;
			}
...
bool MapAndUnits::set_support_to_move_order( PROVINCE_INDEX unit, PROVINCE_INDEX supported_unit, PROVINCE_INDEX destination )
{
	UNITS::iterator unit_to_order;			// The unit to order
	bool unit_ordered = true;				// Whether the unit was ordered successfully
...
bool MapAndUnits::set_move_by_convoy_order( PROVINCE_INDEX unit, PROVINCE_INDEX destination, int number_of_steps, PROVINCE_INDEX step_list[] )
{
	UNITS::iterator unit_to_order;			// The unit to order
	bool unit_ordered = true;				// Whether the unit was ordered successfully
...
bool MapAndUnits::set_move_by_single_step_convoy_order( PROVINCE_INDEX unit, PROVINCE_INDEX destination, PROVINCE_INDEX step )
{
	...
	if ( order_result == TOKEN_ORDER_NOTE_MBV )
	{
		...
		else if ( order_token == TOKEN_ORDER_CTO )
		{
			...
			else
			{
				...
				{
					for ( step_counter = 0; 
						  ( step_counter < convoy_via_list.get_message_length() ) && ( order_result == TOKEN_ORDER_NOTE_MBV ); 
						  step_counter++ )
					{
						convoying_unit_iterator = units.find( convoy_via_list.get_submessage( step_counter ).get_token().get_subtoken() );
		...
		else if ( order_token == TOKEN_ORDER_BLD )
		{
			...
			else
			{
				...
				else
				{
					...
					{
						// Legal build
						winter_record->builds_or_disbands.insert( BUILDS_OR_DISBANDS::value_type( build_location, TOKEN_ORDER_NOTE_MBV ) );
					}
				}
			}
		}
		else if ( order_token == TOKEN_ORDER_REM )
		{
			...
			else
			{
				...
				{
					// Legal removal
					winter_record->builds_or_disbands.insert( BUILDS_OR_DISBANDS::value_type( unit_record->coast_id, TOKEN_ORDER_NOTE_MBV ) );
				}
			}
		}
...
bool MapAndUnits::has_route_to_province( UNIT_AND_ORDER *unit, PROVINCE_INDEX province_index, PROVINCE_INDEX province_to_avoid )
{
